...
ggplot(escorregamento, aes(value)) + 
  geom_histogram(aes(y=..density..), colour="black", fill="steelblue4",binwidth=0.3) +
  geom_density(alpha=0.2, fill="gray")+
      ggtitle("Risk Value")  +
  geom_vline(aes(xintercept=mean(value, na.rm=TRUE)),
            color="firebrick4", linetype="dashed", size=1) +
   geom_vline(aes(xintercept=mean(value, na.rm=TRUE)+sd(value, na.rm=TRUE)),
...
new_escorregamento$risk_level <- cut(new_escorregamento$risk_level, breaks=c(-6,(mean_value-sd_value), mean_value, (mean_value + sd_value),7), labels = c("low", "moderate", "high", "very high" ))
